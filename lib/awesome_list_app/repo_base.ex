defmodule AwesomeListApp.Use.RepoBase do
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      defp apply_function(functions, item) do
        Enum.reduce(functions, fn x, acc -> apply(__MODULE__, x, [acc]) end, item)
      end

      def handle_functions(item, attribute_fields) do
        cond do
          is_list(attribute_fields) ->
            apply_function(attribute_fields, [item])

          is_nil(attribute_fields) ->
            item

          is_atom(attribute_fields) ->
            apply(__MODULE__, attribute_fields, [item])
